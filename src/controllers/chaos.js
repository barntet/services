...
class ChaosController {
    ...
    getSyncErrorThrow = () => {
        throw new Error('Chaos test - sync erro throw');
    }

    getThunkErrorHandle = (req, res, next) => {
        setTimeout(() => {
            next(new Error('Chaos test - thunk erro handle'));
        }, ASYNC_MS);
    }
    ...
    getPromiseErrorThrowWithCatch = callbackcatch(async () => {
        await new Promise(r => setTimeout(r, ASYNC_MS));
        throw new Error('chaos test - primise error throw with catch');
    })
};
